...
for i in range(0,df.size-1):
    for j in range(i+1,df.size-1):
        if df.input[i] + df.input[j] == 2020:
            goodsol += 1
            print("Solution {}. Le produit de {} et {} = {}".format(
    ...
    for j in range(i+1,df.size-1):
        for k in range(j+1, df.size-2):
            if df.input[i] + df.input[j] + df.input[k] == 2020:
                goodsol += 1
                print("Solution {}. Le produit de {} et {} et {} = {}".format(
                    goodsol, df.input[i], df.input[j],df.input[k], df.input[i]*df.input[j]*df.input[k]))